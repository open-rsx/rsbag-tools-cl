...
(defun make-help-string (&key
  ...
  (with-output-to-string (stream)
    (format stream "Capture events being exchanged on the scopes ~
                    designated by URIS and store them in the specified ~
                    output file. For each URI, one or more channels ~
                    may be created in the output file (depending on ~
                    the channel allocation strategy). These channels ~
                    store the events exchanged in the corresponding ~
                    RSB channels.~@
                    ~@
                    ")
...
the available filters. "
  (with-output-to-string (stream)
    (format stream "Specify a filter that received events have to ~
                    ...
                    optional in some cases. Examples (note that the ~
                    single quotes have to be included only when used ~
                    within a shell):~@
                    ~@
                    ~2@T--filter 'origin ~
                    \"EAEE2B00-AF4B-11E0-8930-001AA0342D7D\"'~@
                    ~2@T--filter 'regex \".*foo[0-9]+\"'~@
                    ~2@T--filter 'regex :regex ~
                    \".*foo[0-9]+\"' (equivalent)~@
                    ~2@T-f 'xpath :xpath ~
...
and lists the available strategies."
  (with-output-to-string (stream)
    (format stream "Flush buffers to background storage according to ~
                    the strategy designated by SPEC which has to be of ~
                    the form~@
                    ~@
                    ~2@TKIND KEY1 VALUE1 KEY2 VALUE2 ...~@
                    ~@
                    ...
                    within a shell):~@
                    ~@
                    ~2@T--flush-strategy 'property-limit :property :length/bytes :limit ~
                      33554432'~@
                    ~2@T--flush-strategy ~
                      'property-limit :property :time-to-last-write :limit ~
                      5'~@
                    ~2@T--flush-strategy ~
                      'or (:property-limit :property :time-to-last-write :limit ~
                      5) (:property-limit :property :length/bytes :limit ~
                      33554432)'~@
                    ~@
                    ")
...
(defun make-examples-string (&key
  ...
  (format nil "~2@T~A -o /tmp/everything.tide spread://azurit:4803/~@
               ~@
               Connect to the bus using the Spread transport assuming ~
               the Spread daemon is listening on port 4803 on host ~
               \"azurit\". Participate in the \"root\" ~
               channel (designated by \"/\") and capture all events ~
               into the log file \"/tmp/everything.tide\". Recording ~
               continues until the program receives SIGINT or SIGTERM ~
               each of which causes a clean shutdown.~@
               ~@
               ~2@T~:*~A -o /tmp/nao.tide -f 'xpath :xpath ~
                 \"node()/@width = ~
                 100\" :fallback-policy :do-not-match' ~
                 'spread:/nao/vision/top?name=4803'~@
               ~@
               Store events for the scope \"/nao/vision/top\" (and ~
               sub-scopes) in the log file \"/tmp/nao.tide\", if the ~
               event payload has a width field that has the value ~
               100. The bus connection uses the Spread transport with ~
               the daemon name option.~@
               ~@
               ~2@T~:*~A -o /tmp/multichannel.tide ~
                 'spread:/nao/vision/top' 'spread:/nao/audio/all'~@
               ~@
               Store events being exchanged on the channels designated ~
               by \"/nao/vision/top\" and \"/nao/audio/all\" into the ~
               log file \"/tmp/multichannel.tide\".~@
               "
          program-name))
