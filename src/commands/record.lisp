    ...
    (let+ (((&flet notify (state &optional (value rsb.converter:+no-value+))
      ...
      (rsb.patterns.request-reply:with-methods (server)
          ( ;; Connection level
           ...
           ("info" ()
             (let+ (((&values location channels)
                     (bt:with-lock-held (lock)
                       (check-connection "summarize recorded data")
                       (let* ((bag      (rsbag.rsb:connection-bag connection))
                              (channels (bag-channels bag)))
                         (values (bag-location bag)
               ...
               (json:encode-json-alist-to-string
                `((:location . ,(namestring location))
                  (:channels . ,(mapcar (lambda+ ((name length meta-data))
                                          `((:name   . ,name)
                                            (:count  . ,length)
                                            ,@(plist-alist
                                               (remove-from-plist
